 ...
 * 최소 힙을 구현
 *
 * @param {number[]} operators 연산의 대한 정보. 자연수라면 배열에 x 추가, 0이라면 가장 작은 값을 출력하고 제거
 * @return 0 연산에서 출력한 최솟값, 빈 배열의 경우 0을 출력
 */
function solution(operators) {
  const minHeap = new MinHeap();
  const answer = [];
  operators.forEach((operator) => {
    if (operator) minHeap.insert(operator);
    else answer.push(minHeap.empty() ? 0 : minHeap.extract());
  });
  return answer.join("\n");
}
